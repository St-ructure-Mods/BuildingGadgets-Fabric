...
//                            .build(new MockDelegationWorld(minecraft.world)));
...
//        }
    ...
    private void renderRequirement(PoseStack matrices, int mouseX, int mouseY) {
        ...
        drawString(matrices, minecraft.font, title, 5 - (font.width(title)), 0, Color.WHITE.getRGB());
        ...
        for(Multiset.Entry<IUniqueObject<?>> e: sortedEntries) {
            ...
            if (mouseX > zoneX && mouseX < (zoneX + space) && mouseY > zoneY && mouseY < (zoneY + space)) {
                renderTooltip(matrices, Lists.transform(stack.getTooltipLines(this.minecraft.player, TooltipFlag.Default.NORMAL), Component::getVisualOrderText), x + 15, y + 25);
            }
    ...
    private void pasteTemplateToStack(Level world, ItemStack stack, Template newTemplate, boolean replaced) {
        ITemplateProvider provider = BGComponent.TEMPLATE_PROVIDER_COMPONENT.get(world);
    ...
    private boolean replaceStack() {
        ...
        if (BGComponent.TEMPLATE_KEY_COMPONENT.getNullable(stack) != null)
            return false;
    ...
    private void rename(ItemStack stack) {
        ...
        if(templateProvider != null) {
            ...
            templateProvider.setTemplate(key, template);
            templateProvider.requestRemoteUpdate(key);
        }
    }
...
//                (int) Math.round(minecraft.getWindow().getHeight() - (topPos + panel.getY() + panel.getHeight()) * scale),
...
//        RenderSystem.viewport(0, 0, minecraft.getWindow().getWidth(), minecraft.getWindow().getHeight());
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
        if (panel.contains((int) mouseX - leftPos, (int) mouseY - topPos)) {
            ...
            panelClicked = true;
            clickX = (int) minecraft.mouseHandler.xpos();
            clickY = (int) minecraft.mouseHandler.ypos();
        }
    ...
    protected void renderLabels(PoseStack matrices, int mouseX, int mouseY) {
        if (panelClicked) {
            if (clickButton == 0) {
                float prevRotX = rotX;
                float prevRotY = rotY;
                rotX = initRotX - ((int) minecraft.mouseHandler.ypos() - clickY);
                rotY = initRotY + ((int) minecraft.mouseHandler.xpos() - clickX);
                momentumX = rotX - prevRotX;
                momentumY = rotY - prevRotY;
            } else if (clickButton == 1) {
                panX = initPanX + ((int) minecraft.mouseHandler.xpos() - clickX) / 8f;
                panY = initPanY + ((int) minecraft.mouseHandler.ypos() - clickY) / 8f;
            }
        }
        ...
        if (! nameField.isFocused() && nameField.getValue().isEmpty())
            minecraft.font.draw(matrices, GuiTranslation.TEMPLATE_PLACEHOLDER.format(), nameField.x - leftPos + 4, (nameField.y + 2) - topPos, - 10197916);
...
//    }
    ...
    private void runAfterUpdate(int slot, Runnable runnable) {
        ITemplateKey key = BGComponent.TEMPLATE_KEY_COMPONENT.getNullable(container.getSlot(slot).getItem());
        if(key != null) {
            if(templateProvider != null) {
                templateProvider.registerUpdateListener(new IUpdateListener() {
                    @Override
                ...
                templateProvider.requestUpdate(key);
            }
        }
    }
    ...
    private void onSave() {
        ...
        runAfterUpdate(0, () -> { //we are copying form 0 to 1 => slot 0 needs to be the recent one
            if(templateProvider != null) {
                ITemplateKey key = BGComponent.TEMPLATE_KEY_COMPONENT.getNullable(left);
                if(key != null) {
                    Template templateToSave = templateProvider.getTemplateForKey(key).withName(nameField.getValue());
                    pasteTemplateToStack(templateProvider, right, templateToSave, replaced);
                }
    ...
    private void onLoad() {
        ...
        runAfterUpdate(1, () -> { //we are copying form 1 to 0 => slot 1 needs to be the recent one
            if(templateProvider != null) {
                ITemplateKey key = BGComponent.TEMPLATE_KEY_COMPONENT.getNullable(right);
                if(key != null) {
                    Template templateToSave = templateProvider.getTemplateForKey(key);
                    pasteTemplateToStack(templateProvider, left, templateToSave, replaced);
                }
            }
        });
    }
    ...
    private void onPaste() {
        ...
        String CBString = minecraft.keyboardHandler.getClipboard();
        if (GadgetUtils.mightBeLink(CBString)) {
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_LINK_COPIED.componentTranslation().setStyle(Styles.RED), false);
            return;
        }
        ...
        try {
            ...
            if (!tagFromJson.contains("header")) {
                ...
                minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_WRONG_MC_VERSION
        ...
        try {
            ...
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_SUCCESS.componentTranslation().setStyle(Styles.DK_GREEN), false);
        } catch (CorruptJsonException e) {
            BuildingGadgets.LOG.error("Failed to parse json syntax.", e);
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_CORRUPT_JSON
            ...
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_TOO_RECENT_VERSION
            ...
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_INVALID_JSON
                    .componentTranslation().setStyle(Styles.RED), false);
        } catch (TemplateReadException e) {
            BuildingGadgets.LOG.error("Failed to read Template body.", e);
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED_CORRUPT_BODY
                    .componentTranslation().setStyle(Styles.RED), false);
        } catch (Exception e) {
            BuildingGadgets.LOG.error("Failed to paste Template.", e);
            minecraft.player.displayClientMessage(MessageTranslation.PASTE_FAILED
                    .componentTranslation().setStyle(Styles.RED), false);
        }
    }
}
